...
# ===================== 核心配置项（请根据实际情况修改） =====================
# 原始图片根目录（可通过命令行传参覆盖）
TARGET_DIR = "/data/ocr/zhh/yolo8_bert_cover_detection/samples/yolo"
...
def call_model_api(api_url: str, image_path: str) -> Optional[Dict]:
    ...
    try:
        ...
        # detect_type = result.get("type", "").strip().lower()
        #
        # # 校验返回类型合法性
        # valid_types = {"cover", "category", "first", "other"}
        # if detect_type not in valid_types:
        #     print(f"模型返回非法类型 [{api_url}]：{detect_type} | 图片：{image_path}")
        #     return None
        #
        # return detect_type
...
def copy_and_rename_image(
    ...
    try:
        ...
        # 提取目标目录名
        parent_dirname = get_parent_dirname(src_image_path)
        # 构造新文件名：yolo类型_bert类型_原文件名（无数字后缀）
        new_name = f"{yolo_type}_{bert_type}_{parent_dirname}_{src_path.name}"
...
def traverse_and_process_images(root_dir: str, result_dir: str):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        for filename in files:
            ...
            yolo_result = result.get('yolo_result', None)
            bert_result = result.get('bert_result', None)

            # 3. 拷贝并重命名图片到结果目录
            if copy_and_rename_image(src_image_path, result_dir, yolo_result, bert_result):
                success_count += 1
            else:
                fail_count += 1
